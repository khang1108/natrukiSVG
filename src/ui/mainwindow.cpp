...
void MainWindow::initializeCanvas()
{
    ...
    m_canvas->hide();  // Hidden until document is loaded
    m_canvas->raise(); // Ensure it's on top
}
...
void MainWindow::updateCanvasVisibility()
{
    ...
    m_canvas->setVisible(hasDoc);    // Show canvas if document loaded
    ui->Browse->setVisible(!hasDoc); // Show browse button if no document
}
...
void MainWindow::promptAndLoadSvg()
{
    ...
    if (!filePath.isEmpty()) {
        loadSvgFromFile(filePath); // Load selected file
    }
}
...
void MainWindow::on_Fullscreen_clicked()
{
    if (isFullScreen()) {
        showNormal();
        setFixedSize(m_initialSize); // Restore original size
    }
    else {
        showFullScreen();
    }
